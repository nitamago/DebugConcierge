...
public class Main {
	...
	public void run(String Q_path, String A_path, String Out_path,
		...
		*/
		...
		BaseVisitor visitor = new BaseVisitor(Q_st, Q_end,
				...
				Q_ex, A_ex);
		// オリジナルコードの書き出し
		write_src(Out_path, "q_src.java", q_unit);
		write_src(Out_path, "a_src.java", a_unit);
		...
		try {
			...
			// マスクされたコードの書き出し
			write_src(Out_path, "q_src_masked.java", q_unit);
			write_src(Out_path, "a_src_masked.java", a_unit);
	...
	private void write_src(String out_path, String fname, CompilationUnit unit) {
		try {
			String path = out_path+"/"+fname;
